# THV-0027: AWS STS Authentication for MCP Servers
...
4. **Agent Identity Delegation**: Support for the `act` claim (RFC 7519) is documented as a future extension (see Alternatives)
...
### Alternative 2: Cedar-Based Role Selection

The MVP uses static claim-to-role mappings. A more flexible approach would use Cedar policies to determine which roles a user can assume, based on any combination of claims and context. This would support future scenarios such as:
...
**Proposed design (hybrid Cedar + static config):**

1. **Static config** defines roles with priorities
2. **Cedar policies** determine which roles are permitted for a given user+actor
3. **Selection logic** picks the highest-priority permitted role
...
**Cons:**
- More complex than static mapping
- Overkill if agent delegation not needed

**Why deferred:** The MVP (static role mapping) covers the immediate use case. Cedar-based selection should be implemented when agent delegation becomes a requirement. The design is documented in `docs/aws-sts-agent-delegation-design.md`.
