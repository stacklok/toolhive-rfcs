...
**Planned, not yet shipped:** when more than one upstream provider is configured under `authServerConfig.upstreamProviders` and `XAA.SubjectProviderName` is omitted, configuration validation should fail explicitly, requiring the field to be set — unlike `token_exchange`, which today silently defaults to the first configured upstream in this case via the same shared `InjectSubjectProviderNames` defaulting code path. As of this writing `xaa` shares that same silent-default behavior; [toolhive#5697](https://github.com/stacklok/toolhive/issues/5697) tracks adding the fail-closed check to `xaa` specifically, since it has no existing deployments to break. [toolhive#5687](https://github.com/stacklok/toolhive/issues/5687) tracks the broader defaulting-helper consolidation across `token_exchange`/`aws_sts`/`xaa`.
...
- `ConvertToStrategy` builds a `BackendAuthStrategy` from the CRD, populating the strategy's Step A/B fields (`IDPTokenURL`, `IDPClientID`, `TargetTokenURL`, `TargetClientID`, `TargetAudience`, `TargetResource`, `Scopes`, `SubjectProviderName`, `SubjectTokenType`).
- `ResolveSecrets` reads both `IDPClientSecretRef` and `TargetClientSecretRef` from Kubernetes and resolves their plaintext values directly into the strategy struct (`IDPClientSecret`, `TargetClientSecret`) — not into pod environment variables. `XAAConfig` does carry `IDPClientSecretEnv`/`TargetClientSecretEnv` fields mirroring the env-var pattern other strategies use, but the `xaa` converter does not populate them; secret resolution happens in-process at conversion time instead. Errors from a missing secret or key include the Kubernetes secret's name — it is not redacted — but never its value.
